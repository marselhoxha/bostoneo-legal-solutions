-- Legal Drafting System Tables (MySQL Version)
...
CREATE TABLE legal_documents (
    ...
    expires_at TIMESTAMP NULL, -- For temporary document storage
...
CREATE TABLE drafting_sessions (
    ...
    workflow_state JSON DEFAULT ('{}'), -- Current step, progress, etc.
    context_data JSON DEFAULT ('{}'), -- Collected user responses and context
...
CREATE TABLE document_templates (
    ...
    variables JSON DEFAULT ('{}'), -- Template variable definitions
...
CREATE TABLE citation_cache (
    ...
    is_valid BOOLEAN,
    validation_data JSON DEFAULT ('{}'), -- Detailed validation results
...
CREATE TABLE workflow_steps (
    ...
    questions JSON DEFAULT ('[]'), -- Question definitions
    validation_rules JSON DEFAULT ('{}'),
    is_required BOOLEAN DEFAULT true,
    is_active BOOLEAN DEFAULT true,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY unique_workflow_step (document_type, jurisdiction, workflow_type, step_order)
);
...
INSERT INTO workflow_steps (id, document_type, jurisdiction, workflow_type, step_order, step_id, step_title, step_description, step_type, questions, is_required) VALUES
(UUID(), 'INTERROGATORIES', 'FEDERAL', 'GUIDED_DRAFTING', 1, 'party_info', 'Party Information', 'Identify the parties and your role in the case', 'INFORMATION_GATHERING',
'[
...
(UUID(), 'INTERROGATORIES', 'FEDERAL', 'GUIDED_DRAFTING', 2, 'case_details', 'Case Details', 'Provide information about the legal dispute', 'INFORMATION_GATHERING',
'[
...
(UUID(), 'INTERROGATORIES', 'FEDERAL', 'GUIDED_DRAFTING', 3, 'document_upload', 'Document Upload', 'Upload relevant documents (optional)', 'DOCUMENT_UPLOAD',
'[
...
(UUID(), 'INTERROGATORIES', 'FEDERAL', 'GUIDED_DRAFTING', 4, 'review_generate', 'Review & Generate', 'Review information and generate interrogatories', 'CONTENT_GENERATION', '[]', true);
...
(UUID(), 'Federal Interrogatories Template', 'Standard interrogatories template for federal court', 'INTERROGATORIES', 'FEDERAL', 'Discovery',
