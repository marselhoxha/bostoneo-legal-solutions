...
-- 8. Create stored procedure for daily activity aggregation
DELIMITER //
CREATE PROCEDURE IF NOT EXISTS AggregateDaily AuditActivities()
BEGIN
    DECLARE yesterday DATE;
    SET yesterday = CURDATE() - INTERVAL 1 DAY;
    
    INSERT INTO activity_summary (date, user_id, entity_type, action, activity_count)
    SELECT 
    ...
    GROUP BY DATE(timestamp), user_id, entity_type, action
...
-- 9. Add some sample audit log entries for testing
INSERT INTO audit_log (user_id, action, entity_type, entity_id, description, metadata) VALUES
...
-- 10. Add indexes for performance optimization
CREATE INDEX IF NOT EXISTS idx_audit_log_composite ON audit_log(entity_type, action, timestamp);
CREATE INDEX IF NOT EXISTS idx_case_timeline_composite ON case_timeline(case_id, event_date DESC);
