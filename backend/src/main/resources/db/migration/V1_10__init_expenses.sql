...
CREATE TABLE Expense (
    ...
    FOREIGN KEY (legal_case_id) REFERENCES LegalCase(id) ON DELETE SET NULL,
    ...
    CONSTRAINT chk_expense_reference CHECK (
        (invoice_id IS NULL AND legal_case_id IS NOT NULL) OR
        (invoice_id IS NOT NULL AND legal_case_id IS NULL) OR
        (invoice_id IS NULL AND legal_case_id IS NULL)
    )
);
