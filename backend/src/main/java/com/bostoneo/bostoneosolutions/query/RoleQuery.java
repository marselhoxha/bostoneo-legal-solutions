...
public class RoleQuery {
    ...
    public static final String SELECT_ROLE_BY_USER_ID_QUERY = "SELECT r.id, r.name, r.description, r.hierarchy_level, r.is_system_role FROM roles r JOIN user_roles ur ON ur.role_id = r.id JOIN users u ON u.id = ur.user_id WHERE u.id = :id ORDER BY ur.is_primary DESC, r.hierarchy_level DESC LIMIT 1";
    ...
    public static final String REMOVE_ROLE_FROM_USER_QUERY = "DELETE FROM user_roles WHERE user_id = :userId AND role_id = :roleId";
    
    // Role-Permission queries
    public static final String ASSIGN_PERMISSION_TO_ROLE_QUERY = "INSERT INTO role_permissions (role_id, permission_id) VALUES (:roleId, :permissionId)";
    public static final String REMOVE_PERMISSION_FROM_ROLE_QUERY = "DELETE FROM role_permissions WHERE role_id = :roleId AND permission_id = :permissionId";
    public static final String REMOVE_ALL_PERMISSIONS_FROM_ROLE_QUERY = "DELETE FROM role_permissions WHERE role_id = :roleId";
    public static final String SELECT_PERMISSIONS_BY_ROLE_ID_QUERY = "SELECT p.* FROM permissions p JOIN role_permissions rp ON p.id = rp.permission_id WHERE rp.role_id = :roleId";
    public static final String SELECT_ROLES_BY_USER_ID_QUERY = "SELECT r.* FROM roles r JOIN user_roles ur ON r.id = ur.role_id WHERE ur.user_id = :userId";
    
    // Role CRUD queries
    public static final String INSERT_ROLE_QUERY = "INSERT INTO roles (name, description, hierarchy_level, is_system_role) VALUES (:name, :description, :hierarchyLevel, :isSystemRole)";
    public static final String UPDATE_ROLE_QUERY = "UPDATE roles SET name = :name, description = :description, hierarchy_level = :hierarchyLevel, is_system_role = :isSystemRole WHERE id = :id";
