...
public class ResearchAnalyticsService {
    ...
    private final Map<Long, QueryMetrics> userMetrics = new ConcurrentHashMap<>();
    ...
    public void recordQuery(Long userId, String mode, long executionTimeMs,
        ...
        // Update user metrics
        if (userId != null) {
            QueryMetrics userStats = userMetrics.computeIfAbsent(userId, k -> new QueryMetrics());
    ...
    public Map<String, Object> getCurrentHourAnalytics() {
        String currentHour = LocalDateTime.now().withMinute(0).withSecond(0).withNano(0).toString();
    ...
    public Map<String, Object> getUserAnalytics(Long userId) {
        ...
        QueryMetrics metrics = userMetrics.get(userId);
        if (metrics == null) {
            return Map.of(
                "userId", userId,
                "totalQueries", 0,
    ...
    public Map<String, Object> getSystemCostSummary() {
        double totalCost = userMetrics.values().stream()
            .mapToDouble(m -> m.totalCost)
            .sum();

        int totalQueries = userMetrics.values().stream()
            .mapToInt(m -> m.fastCount + m.thoroughCount)
            .sum();

        int totalCacheHits = userMetrics.values().stream()
            .mapToInt(m -> m.cacheHits)
            .sum();
    ...
    public void resetUserAnalytics(Long userId) {
        userMetrics.remove(userId);
        log.info("🔄 Analytics reset for user: {}", userId);
    }
